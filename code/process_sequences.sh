...
bioawk -cfastx 'END{print NR}' $FILE >> output/SeqSort.txt 
...
bioawk -c fastx '{print $4}' $FILE | awk '{split($0,a,"|");print a[3]}' | sort | uniq -c | sort -nr | >> output/SeqSort.txt

# code to count number of sequences below and above 29,000 bp 
bioawk -cfastx 'BEGIN{shorter = 0} {if (length($2) < 29000) shorter += 1} END {print "Number of sequences shorter than 29K bp", shorter}' $FILE | >> output/SeqSort.txt
bioawk -cfastx 'BEGIN{longer = 0} {if (length($2) >= 29000) longer += 1} END {print "Number of sequences longer than 29K bp", longer}' $FILE | >> output/SeqSort.txt
