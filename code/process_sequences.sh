...
echo "Number of sequences in file:"; bioawk -cfastx "END{print NR}" $FILE > output/SeqSort.txt 
...
"{print $4}" $FILE | awk "{split($0,a,"|");print a[3]}" | sort | uniq -c | sort -nr | tee -a output/SeqSort.txt
...
bioawk -cfastx "BEGIN{longer = 0} {if (length($2) >= 29000) longer += 1} END {print "Number of sequences longer than 29K bp", longer}" $FILE | tee -a output/SeqSort.txt
